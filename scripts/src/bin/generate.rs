...
fn generate_leptos_mod(icon_map: &IconMap) {
	let mut components = Vec::from([
		"// Autogenerated",
		"",
		"use self::util::{ IconPath, IconWeightData };",
		"",
		"pub use self::util::{ Bold, Duotone, Fill, Icon, Light, Regular, Thin };",
		"",
		"mod util;",
		""
	].map(ToOwned::to_owned));
...
fn generate_icon(iconset: &IconSet) -> String {
	...
	for (weight, icon_weight) in weights {
		...
		icon.push_str("_data() -> impl IconWeightData { [");
		for (d, has_opacity) in &*icon_weight.data.paths {
			icon.push_str("IconPath { d: \"");
			icon.push_str(d);
			icon.push_str("\", has_opacity: ");
			icon.push_str(if *has_opacity { "true" } else { "false" });
			icon.push_str(" }, ");
		}

		// cheeky
		let len = icon.len() - 2;
		// SAFETY: trimming off trailing ", " is fine
