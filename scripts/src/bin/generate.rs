...
fn generate_leptos_mod(icon_map: &IconMap) {
	let mut components = Vec::from([
		"// Autogenerated",
		"",
		r#"#![allow(unused_parens, reason = "autogenerated code")]"#,
		"",
		"use super::util::{ Icon, IntoPaths, p, po };",
		""
	].map(ToOwned::to_owned));
	...
	for iconset in icon_map.map.values() {
		...
		components.push(generate_icon(iconset));
	}

	let file = fs::OpenOptions::new()
		.write(true)
		.truncate(true)
		.open("../src/leptos/icons.rs")
		.unwrap();
...
fn generate_icon(iconset: &IconSet) -> String {
	let IconSet { pascalcase_name, bold, duotone, fill, light, regular, thin } = iconset;

	let pascalcase_name = &**pascalcase_name;
	let bold = bold.as_ref().unwrap();
	let duotone = duotone.as_ref().unwrap();
	let fill = fill.as_ref().unwrap();
	let light = light.as_ref().unwrap();
	let regular = regular.as_ref().unwrap();
	let thin = thin.as_ref().unwrap();

	// rather allocate too much, than have to reallocate/copy, I think
	// todo reevaluate this
	let mut icon = String::with_capacity(16000);
	icon.push_str("pub struct ");
	...
	for (weight, icon_weight) in weights {
		icon.push_str("\n\t#[inline(always)] fn ");
		icon.push_str(weight);
		icon.push_str("_data() -> impl IntoPaths { (");
		for (d, has_opacity) in &*icon_weight.data.paths {
			icon.push_str(if *has_opacity { "po" } else { "p" });
			icon.push_str("(\"");
			icon.push_str(d);
			icon.push_str("\"), ");
		}

		if !icon_weight.data.paths.is_empty() {
			// cheeky
			let len = icon.len() - 2;
			// SAFETY: trimming off trailing ", " when it exists is fine
			unsafe { icon.as_mut_vec().set_len(len) }
		}
...
struct IconMap {
	/// maps icon names to their weights (and the data about each weights)
	...
	fn read(
		...
		for style in styles {
			...
			for icon in icons {
				...
				let pascalcase_name = component_name.clone();
				...
				let icon_entry = this.map.entry(icon).or_insert_with(|| IconSet {
					pascalcase_name,
					bold: None,
					duotone: None,
					fill: None,
